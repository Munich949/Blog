<template>
  <el-container v-loading="loading" class="post-article">
    ...
    <el-main class="main">
      <!-- 富文本编辑器 -->
      <div id="editor">
        <mavon-editor style="height: 100%;width: 100%;" ref=md @imgAdd="imgAdd" @imgDel="imgDel"
...
export default {
  ...
  methods: {
    ...
    saveBlog(state) {
      if (!(isNotNullORBlank(this.article.cid))) {
        this.$message({ type: 'error', message: '文章栏目不能为空!' });
        return;
      }
      ...
      if (!(isNotNullORBlank(this.article.mdContent))) {
        this.$message({ type: 'error', message: '文章内容不能为空!' });
        return;
      }
      let _this = this;
      _this.loading = true;
      postRequest("/article/", {
        ...
        // d_render 是 mavon-editor 组件的一个属性，它存储了 Markdown 内容经过解析后的 HTML 字符串。
        // 通过访问 d_render 属性，可以获取到 Markdown 编辑器中编辑的内容的最终渲染结果。
    ...
    imgAdd(pos, $file) {
      ...
      uploadFileRequest("/article/uploadimg", formdata).then(resp => {
        ...
        if (json.status == 'success') {
          ...
          console.log(_this.$refs.md.$imglst2Url([[pos, json.msg]]));
        } else {
