    ...
    <div style="display: flex;justify-content: space-around;flex-wrap: wrap">
      <el-card style="width:330px;margin-top: 10px;" v-for="(user, index) in users" :key="index"
        ...
        <div>
          ...
          <div style="text-align: left;color:#00BFA5;font-size: 12px;margin-top: 13px">
          ...
          <div style="text-align: left;color:#00BFA5;font-size: 12px;margin-top: 13px;display: flex;align-items: center">
            <span>用户状态:</span>
            <el-switch v-model="user.status" active-text="启用" active-color="#13ce66"
              @change="statusChange(user.status, user.id, index)" inactive-text="禁用" style="font-size: 12px">
            </el-switch>
          </div>
          <div style="text-align: left;color:#00BFA5;font-size: 12px;margin-top: 13px">
            <span>用户角色:</span>
            ...
            <el-popover placement="right" title="角色列表" width="200" :key="index + '' + user.id"
              @hide="saveRoles(user.id, index)" trigger="click" v-loading="eploading[index]">
              <el-select v-model="roles" :key="user.id" multiple placeholder="请选择" size="mini">
                <el-option v-for="item in allRoles" :key="user.id + '-' + item.id" :label="item.name" :value="item.id">
                </el-option>
              </el-select>
...
export default {
  ...
  methods: {
    ...
    statusChange(status, id, index) {
      let _this = this;
      _this.cardloading.splice(index, 1, true)
      putRequest("/admin/user/status", { status: status, uid: id }).then(resp => {
        if (resp.status != 200) {
          _this.$message({ type: 'error', message: '更新失败!' })
          _this.loadOneUserById(id, index);
          return;
